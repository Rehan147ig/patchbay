 ...
 * Terminates a non-terminal case as CANCELLED (owner decision). The case
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const remediationCase = await prisma.remediationCase.findFirst({
      ...
      select: { id: true, status: true, reasonCode: true, releaseId: true, repositoryId: true },
    });
    if (!remediationCase) {
    ...
    const updated = await prisma.remediationCase.update({
      ...
      data: {
        ...
        reasonCode: CaseReasonCode.CANCELLED,
        terminalOutcome: CaseTerminalOutcome.CANCELLED,
        terminalAt: new Date(),
      },
    });
