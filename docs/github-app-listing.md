...
| ---------------------------------------------------- | ----------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **DRAFT_PR** (certified patch kits, Node/TypeScript) | `openai`, `stripe`, `twilio`, `anthropic`, `supabase`, `vercel-ai-sdk`  | Patch detects breaking changes and produces **line-level** code patches for these SDKs, verified against the H8 eval corpus (CI gate: 34/34, patches must apply to fixtures). |
...
| **Detect-only**                                      | Python ecosystem                                                        | Patch detects Python SDK releases and assesses their breaking changes, but there is **no Python patch kit** — no Python DRAFT_PR, ever.                                       |
