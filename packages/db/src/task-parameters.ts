 ...
 *
 * A task parameter is one run of the activation task-management loop, scoped
 * to an organization and keyed by (organizationId, taskId, type). Submission
 * is idempotent: retries overwrite inputs, they never duplicate work. A
 * PROCESSING task older than TASK_STALENESS_MS is considered crashed (worker
 * died mid-run) and may be reclaimed by the next submitter or claimer.
 *
 * Every function takes an explicit `organizationId` so cross-organization
 * reads/writes/reclaims are impossible by construction (T6).
 *
...
/**
 * Atomically claims a task parameter for execution within one organization.
 * Claims only PENDING parameters; PROCESSING ones are claimed (reclaimed) only
 * when older than TASK_STALENESS_MS. Returns null when there is nothing to claim.
 */
...
export async function completeTaskParameter(
  ...
  await prisma.taskParameter.updateMany({
    where: {
      organizationId: scope.organizationId,
      taskId: scope.taskId,
      type: scope.type,
...
export async function failTaskParameter(scope: TaskScope, error: string): Promise<void> {
  await prisma.taskParameter.updateMany({
    where: {
      organizationId: scope.organizationId,
      taskId: scope.taskId,
