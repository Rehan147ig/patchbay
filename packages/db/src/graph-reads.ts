 ...
 *
 * Every query is bounded by (organizationId, repositoryId) and only ever
 * reads READY snapshots. Returned objects are plain serializable values, so
 * route handlers can hand them to the client without transformation.
 */
...
export async function latestSnapshot(args: {
  ...
  return prisma.graphSnapshot.findFirst({
...
async function findNodes(args: { where: object }): Promise<GraphNodeRow[]> {
