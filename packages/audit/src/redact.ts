...
const SENSITIVE_KEY_PATTERN =
  /(secret|token|password|passwd|api[_-]?key|apikey|authorization|auth[_-]?header|private[_-]?key|credential|bearer|signature|signing[_-]?secret|webhook[_-]?secret)/i;

/** Common credential formats found in free text (logs, stderr, prompts). */
const SECRET_VALUE_PATTERN =
  /(sk-[a-zA-Z0-9_-]{12,}|gh[pousr]_[a-zA-Z0-9]{20,}|xox[baprs]-[a-zA-Z0-9-]{10,}|AKIA[0-9A-Z]{16}|eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{10,}|-----BEGIN [A-Z ]+ PRIVATE KEY-----|Bearer\s+[a-zA-Z0-9._-]{10,})/g;
